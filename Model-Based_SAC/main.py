...
if __name__ == '__main__':
    ...
    agent = Agent(input_dims=env.state_shape(), n_actions=env.action_size(), episode=Episodes)
    ...
    PV_Rating_history = np.zeros([Time_steps, len(env.PVsystems)])
    ...
    for Time in range(0, Time_steps):
        ...
        env.app.PrintPlain(
            'Time: %s score: %.1f best score: %.1f maximum voltage: %.4f minimum voltage: %.4f avg_P_Out: %s PV_Rating: %s Irradiation: %.4f'
            % (Date_time[Time], score/Episodes, best_score, Vmax, Vmin,  str(np.mean(P_Output)),
               str(np.mean(PV_Rating)), Irradaition))
